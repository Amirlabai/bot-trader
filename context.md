...
- **Resim ledger**: `scratch/resimulate.py` writes `data/ledger_resim.json` (local compare artifact; gitignored). Use `--replace-ledger` only when intentionally overwriting live `ledger.json`. Filter with `--strategy <id>` (repeatable), e.g. `--strategy ma_crypto_long`.
